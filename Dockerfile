...
ENV AIRFLOW_VARIABLES ${AIRFLOW_HOME}/variables
...
RUN set -x && \ 
    ...
    apt-get update && \ 
    DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends -y --force-yes vim-tiny libsasl2-dev libffi-dev gosu krb5-user libcups2 netcat-traditional postgresql-client mysql-client && \
    ...
    /bin/bash -x /usr/local/src/shell/setup-airflow.sh && \
    echo SPARK_HOME is ${SPARK_HOME} && \
    ls -al --g ${SPARK_HOME}

VOLUME ${AIRFLOW_DAG}
VOLUME ${AIRFLOW_VARIABLES}
